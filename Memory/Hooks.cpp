...
#include "../BadMan/Module/Modules/Theme.h"
#include "../BadMan/Module/Modules/HudModule.h"
...
void Hooks::Init() {
	...
	// Signatures
	{
		...
		void* ShulkerBox_isItemAllowed = reinterpret_cast<void*>(FindSignature("48 89 5C 24 ? 57 48 83 EC ? 48 8B 41 ? 48 8B F9 48 85 C0"));
		...
		void* pointerStruct = (void*)FindSignature("40 55 41 54 41 55 48 8D AC 24 ?? ?? ?? ?? 48 81 EC ?? ?? ?? ?? 44 0F 29 A4 24 ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ??");
		g_Hooks.UpdatePointerStructHook = std::make_unique<FuncHook>(pointerStruct, Hooks::UpdatePointerStruct);
		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT){
			...
			if (testMod->isEnabled()) {
				if (testMod->doTranslate)
					matrix = glm::translate<float>(matrix, glm::vec3(testMod->xTrans, testMod->yTrans, testMod->zTrans)); //vec3 side vertical forward (negative for forward pos for back)

				if (testMod->doScale)
					matrix = glm::scale<float>(matrix, glm::vec3(testMod->xMod, testMod->yMod, testMod->zMod));

				if (testMod->doRotate)
					matrix = glm::rotate<float>(matrix, testMod->RotatePos, glm::vec3(testMod->xRotate, testMod->yRotate, testMod->zRotate));
			}
			//matrix = glm::scale<float>(matrix, glm::vec3(0.5f, 0.5f, 1.f)); // vec3 scale the forward scale y scale the horizontal
			//matrix = glm::perspective<float>(100.f, 1.f, 1.f, 1.f);
	...
	if (messageHolder->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			*(DWORD*)(_this + 0xA94) = *(DWORD*)(v17 + 0x20);
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour BadMan prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
...

	{
		...
		if (!g_Data.isInjectorConnectionActive()) {
			...
			if (elapsedFlot > 1.5f) {
				...
				std::string text = "Download the new injector here discord.gg/B6rHK2yXdU";
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic BadMan watermark
			{
				static auto ThemeMod = moduleMgr->getModule<Theme>();
				std::string text = "B A D M A N";
				...
				DrawUtils::drawText(textPos, &text, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat, 1.f), 8.f);
			}
			...
			if (g_Data.allowWIPFeatures()) {
				if (HImGui.Button("Load Script Folder", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
					...
					packet.params[0] = g_Data.addInjectorResponseCallback([](std::shared_ptr<BadManDataPacket> pk) {
			...
			shouldRenderWatermark = TheMarkMod->isEnabled();
...

			{
				...
				if (shouldRenderWatermark) {
...
#elif defined _BETA
					static std::string version = "Private";
#else
					static std::string version = "Public";
#endif
					...
					if (ThemeMod->rainbow) {
						...
						DrawUtils::drawRectangle(rect, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1.f, 1.f);
					}
					DrawUtils::fillRectangle(rect, MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), 1.f);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), nameTextSize);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding + nameLength, rect.w - 7), &version, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), versionTextSize);
				}
				// Draw ArrayList
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					static auto ThemeMod = moduleMgr->getModule<Theme>();
					static auto WelcomeMod = moduleMgr->getModule<Welcome>();
					static auto InvViewMod = moduleMgr->getModule<InventoryView>();
					static auto cpsMod = moduleMgr->getModule<cps>();
					static auto fpsMod = moduleMgr->getModule<fps>();
					static auto keyMod = moduleMgr->getModule<KeyStrokes>();
					static auto arrayMod = moduleMgr->getModule<ArrayList>();
					static auto listMod = moduleMgr->getModule<PlayerList>();
					static auto ArmourMod = moduleMgr->getModule<ArmourHud>();
					static auto CoordsMod = moduleMgr->getModule<Coordinates>();
					static auto ServerInfoMod = moduleMgr->getModule<ServerInfo>();
					static auto pvpMod = moduleMgr->getModule<pvpResources>();
					rainbowArray = arrayMod->rainbowArrayList && arrayMod->isEnabled();
					{
						...
						for (auto it : *moduleList) {
							...
							if (it.get() != hudModule && it.get() != WelcomeMod && it.get() != keyMod && it.get() != arrayMod && it.get() != listMod && it.get() != TheMarkMod && it.get() != ArmourMod && it.get() != CoordsMod && it.get() != ServerInfoMod && it.get() != pvpMod && it.get() != cpsMod && it.get() != fpsMod && it.get() != InvViewMod)
	...
	// Draw Info / Alert Boxes
	{
		...
		if (box) {
			...
			DrawUtils::fillRectangle(rectPos, MC_Color(20, 20, 20), box->fadeVal);
...
float* Hooks::Dimension_getFogColor(__int64 _this, float* color, __int64 a3, float a4) {
	...
	if (CustomSkyMod->isEnabled()) {
		if (CustomSky::custom) {
			...
			currColor[0] = CustomSky::red;
			currColor[1] = CustomSky::green;
			currColor[2] = CustomSky::blue;
...
void Hooks::Actor_lerpMotion(C_Entity* _this, vec3_t motVec) {
	...
	if (noKnockbackmod->isEnabled()) {
		...
		networkSender = (void*)FindSignature("48 8B 74 24 ?? C6 47 ?? ?? 48 8B 5C 24 ??");
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	static auto Packetmod = moduleMgr->getModule<NoMiningTrace>();
	static auto HungerMod = moduleMgr->getModule<AntiHunger>();
	/*static auto funniTextmod = moduleMgr->getModule<TestModule>();

	if (funniTextmod->isEnabled() && g_Data.isInGame()) {
		if (packet->isInstanceOf<C_TextPacket>()) {
			C_TextPacket* funi = reinterpret_cast<C_TextPacket*>(packet);
			std::string msgtext = funi->message.getText();
			funi->message.setText(msgtext + " #BadManOnTop");
		}
	}*/

	if (Packetmod->isEnabled() && packet->isInstanceOf<C_PlayerActionPacket>()) {
		auto* pp = reinterpret_cast<C_PlayerActionPacket*>(packet);

		if (pp->action == 0 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//StartBreak
			return;
		if (pp->action == 1 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//AbortBreak
			return;
		if (pp->action == 2 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//StopBreak
			return;
		if (pp->action == 18 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//ContinueBreak
			return;
	}

	if (HungerMod->isEnabled() && packet->isInstanceOf<C_PlayerActionPacket>()) {
		auto* pp = reinterpret_cast<C_PlayerActionPacket*>(packet);

		if (pp->action == 8 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//jump 
			return;
		if (pp->action == 9 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//Start Sprinting
			return;
		if (pp->action == 10 && pp->entityRuntimeId == g_Data.getLocalPlayer()->entityRuntimeId)//Stop Sprinting
			return;
	}
...
float Hooks::GetGamma(C_Option** optionList) {
	static auto fullBrightModule = moduleMgr->getModule<FullBright>();
	if (fullBrightModule->isEnabled())
		return 25.f;

	moduleMgr->onOptionsTick(optionList);

	auto option = C_Option::getOptionByTranslate(optionList, "options.hidehud");
	if (option != nullptr)
		g_Hooks.shouldRender = !static_cast<bool>(option->valuei);

	//Get the normal gamma value
	float* gamer = (float*)*(optionList + 0x27);
	return *(gamer + 0x7A);
}
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
...

		{
			...
			t->serverName.setText("BadMan Server");
			t->serverName2.setText("BadMan Server");  // This is the one actually displayed
			t->domain.setText(".BadMan.club");
			t->serverAddress.setText("mc.BadMan.club");
...
void Hooks::LocalPlayer__updateFromCamera(__int64 a1, C_Camera* camera) {
	...
	if (freecamMod->isEnabled() && !freecamMod->gCam && g_Data.getLocalPlayer()->isAlive()) {
		// Force camera into position
		memcpy((void*)((uintptr_t)camera + 0xB8), &freecamMod->targetPos, sizeof(vec3_t));

		// make it epic
		camera->renderPlayerModel = true;
		camera->renderFirstPersonObjects = true;

		// Freecam needs this to move properly
		camera->getPlayerRotation(&freecamMod->cameraRot);

		// Dont set player rotation
		return;
		}
	...
	if (tracerMod->isEnabled() && g_Data.isInGame() && g_Data.getLocalPlayer()->isAlive()) {
		memcpy(&tracerMod->cameraPos, (void*)((uintptr_t)camera + 0xB8), sizeof(vec3_t));
		camera->getPlayerRotation(&tracerMod->cameraRot);
	}
...
void Hooks::UpdatePointerStruct(void* a1) {
	auto oFunc = g_Hooks.UpdatePointerStructHook->GetFastcall<void, void*>();

	void* unk = *(void**)((uintptr_t)a1 + 0x140);
	PointingStruct* pointer = Utils::CallVFunc<0x99, PointingStruct*>(unk);

	auto antiAnvilModule = moduleMgr->getModule<AntiAnvil>();

	if (pointer != nullptr && antiAnvilModule->isEnabled()) {
		oFunc(a1);
		...
		if (player != nullptr) {
			...
			int id = (*item->item)->itemId;

			if ((id == 551 || id == 259 || id == 300) && pointer->rayHitType != 1)
				pointer->rayHitType = 3;
		}
		return;
	}
	...
	bool Hooks::ShulkerBlockActor_itemAllowed(C_ItemStackBase* a1) {
	static auto nestMod = moduleMgr->getModule<ShulkerExploit>();
